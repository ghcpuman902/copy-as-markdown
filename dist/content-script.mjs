...
var ParseContext = class {
  ...
  enter(type, attrs2, marks2, preserveWS) {
    let innerMarks = this.findPlace(type.create(attrs2), marks2);
    if (innerMarks)
...
// node_modules/.pnpm/prosemirror-schema-basic@1.2.3/node_modules/prosemirror-schema-basic/dist/index.js
var pDOM = ["p", 0];
...
// node_modules/.pnpm/prosemirror-markdown@1.13.1/node_modules/prosemirror-markdown/dist/index.js
...
// node_modules/.pnpm/prosemirror-tables@1.6.1/node_modules/prosemirror-tables/dist/index.js
var readFromCache;
var addToCache;
...
function findBadColWidths(map2, colWidths, table2) {
  ...
  for (let i = 0; i < map2.map.length; i++) {
    ...
    for (let j = 0; j < attrs2.colspan; j++) {
      ...
      if (colWidth != null && (!attrs2.colwidth || attrs2.colwidth[j] != colWidth))
        (updated || (updated = freshColWidth(attrs2)))[j] = colWidth;
    }
    if (updated)
...
function freshColWidth(attrs2) {
  ...
  const result = [];
  for (let i = 0; i < attrs2.colspan; i++)
    result.push(0);
  return result;
}
...
function setCellAttrs(node, extraAttrs) {
  ...
  if (node.attrs.colspan != 1)
    attrs2.colspan = node.attrs.colspan;
  if (node.attrs.rowspan != 1)
    attrs2.rowspan = node.attrs.rowspan;
  if (node.attrs.colwidth)
    attrs2["data-colwidth"] = node.attrs.colwidth.join(",");
  for (const prop in extraAttrs) {
    const setter = extraAttrs[prop].setDOMAttr;
    if (setter)
      setter(node.attrs[prop], attrs2);
  }
  return attrs2;
}
...
function removeColSpan(attrs2, pos, n = 1) {
  const result = { ...attrs2, colspan: attrs2.colspan - n };
  if (result.colwidth) {
    ...
    for (let row = rect.top; row < rect.bottom; row++) {
      ...
      for (let index = row * map2.width + rect.left, col = rect.left; col < rect.right; col++, index++) {
        ...
        if (extraLeft > 0 || extraRight > 0) {
          ...
          if (extraRight > 0) {
            attrs2 = removeColSpan(
              attrs2,
              attrs2.colspan - extraRight,
              extraRight
            );
          }
          if (cellRect.left < rect.left) {
            ...
            if (!cell) {
              throw RangeError(
                `Could not create cell with attrs ${JSON.stringify(attrs2)}`
              );
            }
          } else {
    ...
    cellAttributes: {
      background: {
        ...
        setDOMAttr(value, attrs2) {
          ...
          if (backgroundColor)
            attrs2.style = (attrs2.style || "") + `background-color: ${backgroundColor};`;
        }
      }
    }
  })),
...
var customMarkdownSerializer = new MarkdownSerializer(
  {
    ...
    table_cell(state, node) {
      node.forEach((paragraph2) => {
        paragraph2.forEach((text2, _, i) => {
          state.render(text2, paragraph2, i);
        });
      });
    },
  ...
  async (message, sender, sendResponse) => {
    if (message.type === "getSelectionHtml") {
      try {
        ...
        console.error("Error processing selection:", error2);
